...
Mediante la definición de una MACRO en el archivo \texttt{board\_api.h} se habilitan funciones de depuración que envían mensajes a través de la UART como puede verse en el algorirmo \ref{lst:debug}.  
...
En primer lugar se definieron dos colas de mensajes idénticas, una para cada procesador, como puede verse en el algoritmo \ref{lst:ipc_queue}.  Cada cola permiten alojar una cantidad \texttt{IPCEX\_QUEUE\_SZ} de mensajes definida mediante una macro con valor numérico igual a 64.
...
Por último, se menciona que en este módulo se implementa el control del perfil de consumo de energía. Se definieron dos perfil distintos con las siguientes características:
...
El sistema se inicia con la configuración del perfil 1 de consumo eléctrico y pude pasar al perfil 2 si el usuario envía una señal de configuración para tal fin a través de la interfaz HMI.  Una vez en el perfil 2, optimizado para \textit{consumo}, sólo es posible volver al perfil 1 reseteando el sistema, ya que la interfaz HMI no se encuentra más disponible.  
%\subsection{Máquina de estados finitos}
